...
function displayPagination(
    ...
    if (strategy === "displayAll") {
        template.forEach((item: string) => handleItemWithParams(item));
    }

    if (strategy === "removeBackButtons") {
        const mapFunction = (item: string) => {
            if (item === "back" && template.includes("back")) {
                return "blank";
            }
            if (item === "maxBack" && template.includes("maxBack")) {
                return "blank";
            }
            return item;
        };

        const mappedTemplate = template.map(mapFunction);
        mappedTemplate.map((item: string) => handleItemWithParams(item));
    }

    if (strategy === "removeNextButtons") {
        const mapFunction = (item: string) => {
            if (item === "next" && template.includes("next")) {
                return "blank";
            }
            if (item === "maxNext" && template.includes("maxNext")) {
                return "blank";
            }
            return item;
        };
